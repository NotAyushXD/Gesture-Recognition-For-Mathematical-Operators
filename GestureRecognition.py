...
while camera.isOpened():
    ...
    if k ==ord('c'):#press 'c' to Clear Screen
        ...
        st = "img/img{}.png".format(count)
        count = count + 1
        crop_img = paintWindow[190:390, 20:250]
        cv2.imwrite(st, crop_img)
        piu.train_and_obtain_number(st[4:-4]) #To get the name without the .jpg or .png extension
        value = piu.getPred()
        cv2.putText(frame, str(value), (0, 0), cv2.FONT_HERSHEY_SIMPLEX, 3, 255)
        bpoints = [deque(maxlen=512)]
        gpoints = [deque(maxlen=512)]
        rpoints = [deque(maxlen=512)]
        ypoints = [deque(maxlen=512)]
